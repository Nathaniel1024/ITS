...
function getStatusClass(status) {
  ...
  if (["on progress", "for review", "released"].includes(status)) return "status-orange";
  if (["approved", "completed"].includes(status)) return "status-green";
  if (["returned", "cancelled"].includes(status)) return "status-red";
  return "";
}
  ...
  trackers.forEach((tracker, idx) => {
    ...
    tbody.innerHTML += `
      <tr>
        ...
        <td>
          <button class="action-btn" onclick="viewTracker(${idx})">View Details</button>
        </td>
      </tr>
    `;
  });
...
function searchManualTrackers() {
  ...
  const filtered = manualTrackers.filter(tracker => {
    const matchesSearch =
      tracker.fundName.toLowerCase().includes(search) ||
      tracker.id.toLowerCase().includes(search);
    const matchesStatus = !status || tracker.status === status;
    return matchesSearch && matchesStatus;
  });
  renderManualTrackerList(filtered);
}
